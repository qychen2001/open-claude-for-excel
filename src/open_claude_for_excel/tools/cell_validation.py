...
def get_data_validation_for_cell(
    worksheet: Worksheet, cell_address: str
...
def _cell_in_validation_range(row: int, col: int, data_validation) -> bool:
    ...
    try:
        ...
        logger.warning(
            f"Error checking if cell ({row}, {col}) is in validation range for DV sqref '{getattr(data_validation, 'sqref', 'N/A')}': {e}"
        )
        return False


def _extract_validation_metadata(
    data_validation, cell_address: str, worksheet: Optional[Worksheet] = None
) -> Dict[str, Any]:
...
def _extract_list_values(
    formula: str, worksheet: Optional[Worksheet] = None
) -> List[str]:
    ...
    try:
        ...
        # Handle range reference (e.g., "$A$1:$A$5" or "Sheet1!$A$1:$A$5")
            ...
            try:
                ...
                logger.warning(
                    f"Could not resolve range '{formula}' for list validation: {e}"
                )
    ...
    try:
        for dv in worksheet.data_validations.dataValidation:
            ...
            if dv.type == "list" and dv.formula1:
                validation_info["allowed_values"] = _extract_list_values(
                    dv.formula1, worksheet
                )
